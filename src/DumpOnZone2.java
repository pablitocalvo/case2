...
{		
		...
		Elements annunci = doc.select("li[data-id].listing-item");
		
		//System.out.println("pag.="+k+"  "+ annunci.size());
		//if (true) continue;
...
//		Elements stanze = annunci.select(".listing-features__room").select("div");
...
//			    return; }
		...
		for (int i = 0; i < annunci.size() ; i++) 
		{
			...
			String linkCasa= annuncio.select("a").attr("href");
			...
			String prezzo = annuncio.select(".listing-features__price").html().replaceAll("[^0-9]", "");
